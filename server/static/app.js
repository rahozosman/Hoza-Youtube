...
(() => {
  ...
  /* The session token of a packaged installation. It is delivered in the
     ...
     none, so this is empty and nothing changes. */
  const TOKEN = document.querySelector('meta[name="hoza-token"]')?.content || "";

  /** `EventSource` cannot carry a header, so the one endpoint it opens takes
      the token as a query parameter instead. It never leaves this machine. */
